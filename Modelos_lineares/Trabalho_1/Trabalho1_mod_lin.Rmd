...
#inserir imagem de grupos de preditores
...
No entanto, muitas das variáveis preditoras estão altamente correlacionadas entre si, de modo que não parece apropriado incluir todas no mesmo modelo.


Segundo o quadro teórico, excluímos as variáveis incidência (pois já está incluída na variável de prevalência) e a variável yrslost (pois já está incluída em DALYs).
DALYs e hospdays são medidas de morbidade. Acreditamos que DALY é uma medida mais abrangente e excluímos hospdays.
No entanto, nosso receio é de haver correlação entre mort e disabil, dado que os DALYs incluem morte em anos de vida perdidos.
Assim, uma solução pode ser pegar apenas o componente de morbidade da medida DALY, criando a variável YLD = DALY - yrslost.

Vamos ao modelo com preval, mort e YLD (descobrir como criar a nova variável)

Ver multicolinearidade. 

Outras ideias: modelo com yrslost dividido por mort (anos de vida perdidos em média por pessoa), prevalência da doença (medida de quantidade) e YLD dividido por prevalencia (anos vividos com incapacidade ajustados para prevalência da doença)

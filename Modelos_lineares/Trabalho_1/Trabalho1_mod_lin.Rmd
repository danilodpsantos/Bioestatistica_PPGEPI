...
```{r agrupando_diseases, message=FALSE, warning=FALSE, include=FALSE, paged.print=TRUE}
...
```{r descritiva_ex_1, fig.width=7, message=FALSE, warning=FALSE, paged.print=TRUE}
...
Sugiro uma descritiva para verificar se o desfecho apresenta distribuição normal (já que estamos usando regressão linear)
...
```{r distribution_incid}

#acho que seria melhor mostrar a distribuição da incidência, como é variável contínua
hist(dados$incid, breaks = 50)

```

```{r distribution_incid_2}
...
```{r distribution_preval}

hist(dados$preval, breaks = 50)
summary(dados$preval)
```
...
```{r distribution_hospdays}

hist(dados$hospdays, breaks = 50)
summary(dados$hospdays)
```
...
```{r distribution_yrslost}

hist(dados$yrslost, breaks = 50)

summary(dados$yrslost)
```
...
```{r distribution_disabil}

hist(dados$disabil, breaks = 50)
summary(dados$disabil)
...
* 1) quantidade de pessoas acometidas pela doença, 
* 2) morbidade da doença e 
* 3) mortalidade da doença. 
...
Métodos backward, forward e stepwise não nos parecem adequados, devido à alta correlação entre os preditores, indicando colinearidade.
...
```{r residuos_student_qq}

car::qqPlot(modelo1, main = "QQ Plot")
...
# Preparando modelo com todas as variáveis a serem avaliadas
modelo_full <- lm(data = dados, log(dollars) ~ log(incid) + log(preval) + log(hospdays) + log(mort) + log(yrslost) + log(disabil) + log(YLD))
 
# Conferindo resultado deste modelo                 
summary(modelo_full)

# Método stepwise de seleção de variáveis:

step.model <- stepAIC(modelo_full, direction = "both", trace = TRUE)
...
Não consegui explicar direito, achei melhor nem escrever :/
...
stepwise_mrm_3 <- lm(data = dados, formula = (log(dollars) ~ log(disabil) + log(incid)))
...
stepwise_mrm_4 <- lm(data = dados, formula = (log(dollars) ~ log(disabil) + log(mort)))
...
stepwise_mrm_5 <- lm(data = dados, formula = (log(dollars) ~ log(disabil) + log(hospdays)))
...
stepwise_mrm_6 <- lm(data = dados, formula = (log(dollars) ~ log(disabil) + log(hospdays) + log(preval)))
...
#acho que tem outro jeito de fazer também, plotando em um gráfico os dados salvos nos objetos abaixo

dados$predicted <- predict(stepwise_mrm_7)
dados$studentized <- rstudent(stepwise_mrm_7)
...
```
Será que aqui não devemos usar o intervalo de predição no lugar do intervalo de confiança? Acho que o de confiança é na questão de baixo e o de predição é aqui :)

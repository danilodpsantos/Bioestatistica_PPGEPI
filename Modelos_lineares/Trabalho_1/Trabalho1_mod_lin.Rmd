...
O objetivo do nosso modelo é investigar quais características das doenças estão associadas com o investimento dado a elas pelo NIH.
Nossa hipótese é que o investimento em pesquisa do NIH pode ser explicado pela quantidade de pessoas acometidas pela doença, além de sua morbidade e mortalidade. 
Todas as variáveis preditoras estão relacionadas a esses três aspectos. 
No entanto, como vimos no passo anterior, muitas das variáveis preditoras estão altamente correlacionadas entre si, de modo que não parece apropriado incluir todas no mesmo modelo.

Nossa estratégia para realizara modelagem e definir quais variáveis manter será a __manual__. Após fazer modelos univariáveis (questão anterior), passamos a um modelo multivariável, inicialmente com todas as variáveis.

```{r mr_dol_1}

mr_dol_1 <- lm(data = dados, formula = (dollars ~ incid + preval + mort + hospdays+ yrslost + disabil))
